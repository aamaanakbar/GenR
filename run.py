...
def run_phase(label: str, variable: Variable, lr: float):        
    ...
    try:
        for _ in tqdm.tqdm(range(150), desc=label):
            x = variable.to_image()  # Predicted image
            loss = loss_fn(degradation.degrade_prediction, x, target, degradation.mask).mean()  # Loss calculation
    ...
    # Degradation of prediction and ground truth for visual analysis
        ...
        with directory(experiment_path):
            ...
            for j, image_path in enumerate(image_paths):
                with directory(f"inversions/{j:04d}"):
                    ...
                    # Start optimization with different variable stages
